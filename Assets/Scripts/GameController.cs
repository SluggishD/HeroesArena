...
public class GameController : MonoBehaviour
{
    ...
    public void UnitGetTargetPos(Unit unit,bool isOrange)
    {
        ...
        //if (isOrange)
        //{
        //    buildings = orangeBuildings;
        //}
        //else
        //{
        //    buildings = purpleBuildings;
        //}
        ...
        //国王没死 根据当前单位X坐标和国王对比进行判断,群体单位则取父类进去对比

        int index;
        if (unit.transform.parent == null)
        {
            index = unit.transform.position.x <= buildings[0].transform.position.x ? 1 : 2;//判断单位是靠近左边还是右边
        }
        else
        {
            index = unit.transform.parent.position.x <= buildings[0].transform.position.x ? 1 : 2;//判断单位是靠近左边还是右边
        }
